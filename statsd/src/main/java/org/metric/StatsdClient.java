...
public class StatsdClient extends TimerTask {
    ...
    public static void main(String[] arg){
        while(true){
            ...
            try {
                ...
                //client.increment("wpk.business.increment", 50);
                //client.gauge("wpk.business.gauge", 80);
                cpu(client);
    ...
    private static void cpu(StatsdClient client) throws SigarException, IOException {
        Sigar sigar = new Sigar();
        CpuInfo[] infos = sigar.getCpuInfoList();
        CpuPerc[] cpuList = sigar.getCpuPercList();
        for (int i = 0; i < infos.length; i++) {// 不管是单块CPU还是多CPU都适用
            sendCpuPercToGraphite(cpuList[i], client, i+1);
            client.flush();
        }
    }

    private static void sendCpuPercToGraphite(CpuPerc cpu, StatsdClient client, int num) throws IOException {
        //CPU用户使用率
        String cpuUser = CpuPerc.format(cpu.getUser()).replace("%","");
        client.gauge(String.format("wpk.windows.%s.cpu_%s","cpuUser",num), Double.parseDouble(cpuUser));

        //CPU系统使用率
        String cpuSys = CpuPerc.format(cpu.getSys()).replace("%","");
        client.gauge(String.format("wpk.windows.%s.cpu_%s","cpuSys",num), Double.parseDouble(cpuSys));

        //CPU当前等待率
        String cpuWait = CpuPerc.format(cpu.getWait()).replace("%","");
        client.gauge(String.format("wpk.windows.%s.cpu_%s","cpuWait",num), Double.parseDouble(cpuWait));

        //CPU当前错误率
        String cpuNice = CpuPerc.format(cpu.getNice()).replace("%","");
        client.gauge(String.format("wpk.windows.%s.cpu_%s","cpuNice",num), Double.parseDouble(cpuNice));

        //CPU当前空闲率
        String cpuIdle = CpuPerc.format(cpu.getIdle()).replace("%","");
        client.gauge(String.format("wpk.windows.%s.cpu_%s","cpuIdle",num), Double.parseDouble(cpuIdle));

        //CPU总的使用率
        String cpuCombined = CpuPerc.format(cpu.getCombined()).replace("%","");
        client.gauge(String.format("wpk.windows.%s.cpu_%s","cpuCombined",num), Double.parseDouble(cpuCombined));
    }
}
